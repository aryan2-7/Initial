 ...
 * webcam_test.cpp
 * ───────────────
 * A simple test to confirm OpenCV is installed correctly
 * and your webcam is accessible.
 *
 * Place this file in:  tests/webcam_test.cpp
 *
 * Build & run:
 *   cmake --build . --target webcam_test
 *   ./webcam_test          (Linux/macOS)
 *   webcam_test.exe        (Windows)
 *
 * Controls:
...
int main() {
    ...
    while (true) {
        ...
        // 0xFF mask is needed on some Linux systems
